...
impl Curvature {
    ...
    pub fn from_frames(curr: Frame, prev: Frame) -> Self {
        ...
        if diff.magnitude() < sim::EPSILON {
            return Self::new(0.0, 0.0, curr.pitch().abs().cos(), 0.0);
        }

        let delta_pitch = sim::wrap_angle(curr.pitch() - prev.pitch());
        let delta_yaw = sim::wrap_angle(curr.yaw() - prev.yaw());
        let yaw_scale = curr.pitch().abs().cos();
        let total_angle = (yaw_scale * yaw_scale * delta_yaw * delta_yaw + delta_pitch * delta_pitch).sqrt();
...
mod tests {
    ...
    fn from_euler(pitch: f32, yaw: f32, roll: f32) -> Frame {
        ...
        let roll_quat = crate::Quaternion::from_axis_angle(direction, -roll);
