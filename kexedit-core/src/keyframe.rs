...
pub fn evaluate_segment(start: &Keyframe, end: &Keyframe, t: f32) -> f32 {
    ...
    if matches!(end.in_interpolation, InterpolationType::Constant) {
        return end.value;
    }
...
fn evaluate_bezier_2d(start: &Keyframe, end: &Keyframe, target_time: f32) -> f32 {
    ...
    for _ in 0..8 {
        ...
        let bezier_time_derivative = -3.0 * one_minus_u2 * p0_x
            + 3.0 * one_minus_u2 * p1_x
...
mod tests {
    ...
    fn evaluate_bezier_interpolation_smooth_curve() {
        ...
        let result = evaluate(keyframes, 0.5, 0.0);
        assert!(result > 0.0, "Bezier should produce positive value at midpoint");
        assert!(result < 1.0, "Bezier should produce value less than end at midpoint");
    }

    #[test]
    fn evaluate_bezier_interpolation_with_tangents() {
        let keyframes = &[
            Keyframe::new(
                0.0,
                0.0,
        ...
        let result_at_25 = evaluate(keyframes, 0.25, 0.0);
        let result_at_75 = evaluate(keyframes, 0.75, 0.0);

        assert!(result_at_25 > 0.0, "Bezier should produce positive values");
        assert!(result_at_75 < 1.0, "Bezier should stay within bounds");
        assert!(
            result_at_75 > result_at_25,
    ...
    #[test]
    fn evaluate_segment_constant_out_interpolation_returns_start_value() {
        let start = Keyframe::new(
            0.0,
            100.0,
            InterpolationType::Bezier,
            InterpolationType::Constant,
        ...
        let end = Keyframe::simple(1.0, 200.0);

        let result = evaluate_segment(&start, &end, 0.5);
        assert_relative_eq!(result, 100.0, epsilon = TOLERANCE);
    }

    #[test]
    fn evaluate_segment_constant_in_interpolation_returns_end_value() {
        let start = Keyframe::simple(0.0, 100.0);
        ...
        let result = evaluate_segment(&start, &end, 0.5);
        assert_relative_eq!(result, 200.0, epsilon = TOLERANCE);
    }
    ...
    #[test]
    fn evaluate_bezier_convergence_large_weights() {
        let keyframes = &[
            Keyframe::new(
                0.0,
                0.0,
        ...
        let mut t = 0.0;
        while t <= 1.0 {
            let _result = evaluate(keyframes, t, 0.0);
            t += 0.1;
        }
    }
}
